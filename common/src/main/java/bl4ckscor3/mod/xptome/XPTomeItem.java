...
public class XPTomeItem extends Item {
	...
	public InteractionResult use(Level level, Player player, InteractionHand hand) {
		...
		if (player.isShiftKeyDown() && storedXP < getMaxXP(stack)) {
			...
			if (stack.has(XPTome.STORE_UNTIL_PREVIOUS_LEVEL.get())) {
			...
			boolean asOrbs = stack.has(XPTome.RETRIEVE_XP_ORBS.get());

			if (stack.has(XPTome.RETRIEVE_UNTIL_NEXT_LEVEL.get())) {
	...
	private void addOrSpawnXPForPlayer(Player player, int amount, boolean asOrbs) {
		if (asOrbs) {
			...
			XPTome.platform().onXpChange(player, amount);
	...
	public void appendHoverText(ItemStack stack, TooltipContext ctx, TooltipDisplay display, Consumer<Component> tooltipAdder, TooltipFlag flag) {
		...
		if (stack.has(XPTome.RETRIEVE_XP_ORBS.get()) && display.shows(XPTome.RETRIEVE_XP_ORBS.get()))
	...
	public static void setStoredXP(ItemStack stack, int amount) {
		stack.set(XPTome.STORED_XP.get(), amount);
	}
	...
	public static int getStoredXP(ItemStack stack) {
		return stack.getOrDefault(XPTome.STORED_XP.get(), 0);
	}
	...
	public static int getMaxXP(ItemStack stack) {
		return stack.getOrDefault(XPTome.MAXIMUM_XP.get(), DEFAULT_MAX_XP);
	}
	...
	public static double getRetrievalPercentage(ItemStack stack) {
		return stack.getOrDefault(XPTome.RETRIEVAL_PERCENTAGE.get(), 1.0D);
	}
}
