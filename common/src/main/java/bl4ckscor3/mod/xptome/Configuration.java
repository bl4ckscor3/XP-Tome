...
public class Configuration {
	...
	private final BooleanValue retrieveUntilNextLevel;
	...
	public Item.Properties applyToDefaultComponents(Item.Properties properties) {
		properties.component(XPTome.MAXIMUM_XP.get(), maxXP.getAsInt());
		properties.component(XPTome.RETRIEVAL_PERCENTAGE.get(), retrievalPercentage.getAsDouble());

		if (retrieveXPOrbs.getAsBoolean())
			properties.component(XPTome.RETRIEVE_XP_ORBS.get(), Unit.INSTANCE);

		if (retrieveUntilNextLevel.getAsBoolean())
			properties.component(XPTome.RETRIEVE_UNTIL_NEXT_LEVEL.get(), Unit.INSTANCE);

		if (storeUntilPreviousLevel.getAsBoolean())
			properties.component(XPTome.STORE_UNTIL_PREVIOUS_LEVEL.get(), Unit.INSTANCE);
